...
export const AssetCategoryView: React.FC<AssetCategoryViewProps> = ({
  ...
  const categoryGroups: CategorySummary[] = Object.values(ASSET_CATEGORIES)
    .map(categoryInfo => {
      const categoryAssets = assets.filter(
        asset => asset.category === categoryInfo.id
      );
  ...
  const totalPortfolioValue = categoryGroups.reduce(
    (sum, group) => sum + group.totalValue,
    0
  );
  const totalZakatEligible = categoryGroups.reduce(
    (sum, group) => sum + group.zakatEligibleValue,
    0
  );
    ...
    <div className="space-y-6">
      ...
      <div className="space-y-4">
        {categoryGroups.map(group => {
          ...
          const categoryInfo =
            ASSET_CATEGORIES[
              group.categoryId.toUpperCase() as keyof typeof ASSET_CATEGORIES
            ];
          const percentage =
