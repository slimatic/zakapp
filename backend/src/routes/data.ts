...
router.post('/import', authenticateToken, async (req: AuthenticatedRequest, res) => {
  try {
    ...
    let importedCount = 0;
    const importResults: Record<string, { success: boolean; error?: string }> = {};
    ...
    for (const [filename, fileData] of Object.entries(userData)) {
      ...
      try {
        if (mergeStrategy === 'replace') {
          ...
          await writeUserFile(user.userId, filename, fileData as Record<string, unknown>);
        } else {
          ...
          const mergedData = { ...existingData, ...(fileData as Record<string, unknown>) };
 ...
 */
router.post('/backup', authenticateToken, async (req: AuthenticatedRequest, res) => {
  try {
    const user = req.user;
    if (!user) {
      return res.status(401).json({
