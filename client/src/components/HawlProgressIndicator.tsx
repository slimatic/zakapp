...
export const HawlProgressIndicator: React.FC<HawlProgressIndicatorProps> = ({
  ...
  const getProgressColor = (): string => {
    if (record.status !== 'DRAFT') return 'bg-border-strong';
    if (progressPercent >= 100) return 'bg-success';
    if (progressPercent >= 75) return 'bg-warn';
    if (progressPercent >= 50) return 'bg-warn';
    return 'bg-secondary';
  };
        ...
        <div className="rounded-lg border border-border bg-muted p-4">
          <div className="flex items-center justify-between">
            ...
            {record.status === 'FINALIZED' && (
              <div className="rounded-full bg-success-soft px-3 py-1">
                <span className="text-xs font-medium text-success">Finalized</span>
              </div>
            )}
            {record.status === 'UNLOCKED' && (
              <div className="rounded-full bg-warn-soft px-3 py-1">
                <span className="text-xs font-medium text-warn-strong">Unlocked</span>
              </div>
            )}
          </div>
        </div>
    ...
    <div className={`hawl-progress-indicator hawl-status-draft ${className}`}>
      <div className="rounded-lg border border-border bg-accent p-4">
        ...
        <div className="mb-4 flex items-center justify-between">
          <h3 className="text-sm font-semibold text-foreground">Hawl Progress</h3>
          <div className="flex items-center gap-2">
            {isUpdating && (
              <span className="inline-flex items-center gap-1 rounded-full bg-accent px-2 py-1 text-xs font-medium text-secondary">
        ...
        <div className="mb-4 grid grid-cols-3 gap-4 text-center">
          <div className="rounded-lg bg-card p-3">
            <div className="text-2xl font-bold text-secondary">{daysRemaining}</div>
            <div className="text-xs text-muted-foreground">Days Remaining</div>
          </div>
          <div className="rounded-lg bg-card p-3">
            <div className="text-2xl font-bold text-foreground/80">{daysElapsed}</div>
            <div className="text-xs text-muted-foreground">Days Elapsed</div>
          </div>
          <div className="rounded-lg bg-card p-3">
            <div className="text-2xl font-bold text-foreground/80">{progressPercent.toFixed(0)}%</div>
        ...
        <div className="grid grid-cols-2 gap-3 border-t border-border pt-3">
          <div>
            ...
            <div className="text-sm text-foreground">{formatDate(startDate)}</div>
            <div className="text-xs text-muted-foreground">(Gregorian)</div>
          </div>
          <div>
            <div className="text-xs font-medium text-muted-foreground">Completion Date</div>
            <div className="text-sm text-foreground">{formatDate(completionDate)}</div>
