  ...
  return (
    <div className="bg-card text-card-foreground border border-border rounded-lg p-4 md:p-5 shadow-sm" data-testid="payment-history-card">
      <div className="flex items-center justify-between mb-4">
        ...
        <span className={`text-xs px-2 py-0.5 rounded-full ${isFullyPaid ? 'bg-success-soft text-success' : 'bg-accent text-secondary'}`}>
      ...
      <div className="space-y-2">
        ...
        <div className="border-t border-border pt-2 flex justify-between text-sm">
          ...
          <span className={`font-bold ${remainingBalance === 0 ? 'text-success' : 'text-danger'}`}>
      ...
      {canRecordPayment && (
        <button
          ...
          className="mt-4 w-full px-4 py-2 bg-success text-success-foreground rounded-lg text-sm font-medium hover:bg-success/90 transition-colors"
        >
          + Record Payment
        </button>
      )}
