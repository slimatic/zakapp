...
describe('AssetSelectionTable (#310 round 6)', () => {
  ...
  it('formats IDR when currency="IDR" prop is passed', () => {
    ...
    // Use a custom function matcher to reliably detect the formatted value.
    const idrFormatted = screen.getAllByText((content) => {
      return (content || '').includes('IDR') && (content || '').includes('1,000,000');
    });
