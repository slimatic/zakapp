...
export const PaymentImportExport: React.FC = () => {
  ...
  return (
    <div className="max-w-4xl mx-auto p-6">
      <div className="mb-8">
        <h1 className="text-3xl font-bold text-foreground mb-4">Import & Export Payments</h1>
        <p className="text-lg text-muted-foreground">Backup or import Zakat payment records for tracking and reporting.</p>
      </div>

      {/* Export Section */}
      <div className="bg-card rounded-lg border border-border p-6 mb-8">
        <h2 className="text-xl font-semibold text-foreground mb-4">Export Payments</h2>
        <p className="text-muted-foreground mb-6">Download your payments in CSV or JSON format. Currently you have <strong>{payments.length}</strong> payments.</p>
        ...
        {payments.length === 0 && (
          <div className="mt-4 text-sm text-warn-strong bg-warn-soft border border-warn/30 rounded-lg p-3">No payments to export.</div>
        )}
      </div>

      {/* Import Section */}
      <div className="bg-card rounded-lg border border-border p-6 mb-8">
        <h2 className="text-xl font-semibold text-foreground mb-4">Import Payments</h2>
        <p className="text-muted-foreground mb-6">Upload a CSV or JSON file to import multiple payments at once. Each payment must include a `snapshotId` (Nisab Year Record).</p>
        ...
        {importing && (
          <div className="flex items-center space-x-2 text-secondary"><LoadingSpinner size="sm" /><span>Importing payments...</span></div>
        )}
        ...
        {importResult && (
          <div className={`rounded-lg p-4 ${importResult.failed === 0 ? 'bg-success-soft border border-success/30' : 'bg-warn-soft border border-warn/30'}`}>
            <h3 className={`font-semibold ${importResult.failed === 0 ? 'text-success' : 'text-warn-strong'}`}>Import Results</h3>
            <div className="mt-2 text-sm">
              <p className="text-success">✅ Successfully imported: {importResult.success} payments</p>
              {importResult.failed > 0 && (<p className="text-danger">❌ Failed to import: {importResult.failed} payments</p>)}
            </div>
            ...
            {importResult.errors.length > 0 && (
              <div className="mt-4">
                ...
                <ul className="text-sm text-danger space-y-1">
                  ...
                  {importResult.errors.length > 5 && (<li className="text-danger">...and {importResult.errors.length - 5} more errors</li>)}
                </ul>
              </div>
            )}
          </div>
