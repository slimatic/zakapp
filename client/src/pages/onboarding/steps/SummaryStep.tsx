...
export const SummaryStep: React.FC = () => {
    ...
    const handleFinish = async () => {
        ...
        try {
            ...
            const createdLiabilitiesList: any[] = [];
            try {
                ...
                const liabilityPromises = (Object.entries(data.liabilities || {}) as [string, AssetData][])
                    ...
                    .map(([key, assetData]) => {
                        ...
                        createdLiabilitiesList.push({ ...newLiability, id: `temp-liab-${key}` });

                        return addLiability(newLiability);
                    });
            ...
            // Fix: Use standard calculator to ensure Total vs Zakatable is correct
            const allAssetsForCalc = [...existingAssets, ...createdAssetsList];
            const allLiabilitiesForCalc = [...existingLiabilities, ...createdLiabilitiesList];

            const { totalWealth, zakatableWealth, netZakatableWealth } = calculateWealth(allAssetsForCalc, allLiabilitiesForCalc);

            const calculatedZakatAmount = (netZakatableWealth >= nisabThreshold) ? (netZakatableWealth * 0.025) : 0;
            ...
            try {
                ...
                const hawlDoc = await addRecord({
                    ...
                    totalWealth: totalWealth, // Gross Assets
                    zakatableWealth: netZakatableWealth, // Net Zakatable
    ...
    if (postOnboardingStep === 'wealth') {
        // Since we have completed onboarding and added assets to the DB,
        // existingAssets should now reflect the COMPLETE portfolio (including new items).
        // We use createdRecord for the precise snapshot of wealth calculated at the moment of creation.

        const finalTotalWealth = createdRecord?.totalWealth || 0;
        const finalZakatableWealth = createdRecord?.zakatableWealth || 0;
        ...
        return (
            <div className="space-y-6">
                ...
                <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-4">
                    {/* existingAssets is from the repository hook, which updates live after our inserts */}
                    <AssetsBreakdownChart assets={existingAssets} currency="USD" />
                </div>
