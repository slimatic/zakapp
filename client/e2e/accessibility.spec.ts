...
const ACCESSIBILITY_STANDARDS = {
  violations: 50, // Allow many violations for basic smoke test
  incomplete: 50, // Allow many incomplete checks
  passes: 3 // Minimum number of successful accessibility checks
};
...
test.describe('Accessibility Compliance Tests', () => {
  ...
  test.describe('Public Pages Accessibility', () => {
    ...
    test('Landing page should load and be testable', async () => {
      await page.goto('/');
      ...
      expect(accessibilityScanResults.violations.length).toBeLessThanOrEqual(ACCESSIBILITY_STANDARDS.violations);
      expect(accessibilityScanResults.passes.length).toBeGreaterThanOrEqual(
        ACCESSIBILITY_STANDARDS.passes
      );
    });

    test('Login page should load and be testable', async () => {
      ...
      // Basic check that page loaded (even if showing error)
      await expect(page.locator('body')).toBeVisible();
    });

    test('Registration page should load and be testable', async () => {
  ...
  test.describe('Dashboard and Navigation Accessibility', () => {
    test.beforeEach(async ({ browser }) => {
      ...
      // For protected pages, we need to skip authentication since API is disabled
      // Instead, navigate directly to the page assuming it's accessible
      // In a real scenario, you'd mock the auth or use a test user
    });
